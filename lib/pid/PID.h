...
{
public:
  PIDController(double p, double i, double d, boost::function<T(void)> pidSource, boost::function<void(T)> pidOutput);
  void tick();
  void setTarget(T t);
  T getTarget();
  T getOutput();
  ...
  double getI();
  double getD();
  void setPIDSource(boost::function<T(void)> pidSource);
  void setPIDOutput(boost::function<void(T)> pidOutput);
...
private:
  ...
  boost::function<T(void)> _pidSource;
  boost::function<void(T)> _pidOutput;
